...
select 
    ...
    map_cities.state,
    map_cities.region,
    map_cities.updated_date as pickup_city_updated_date,

    map_cancellation_reasons.cancellation_reason

from STREAM(realtimedata.bronze.stg_rides) stg_rides

LEFT JOIN realtimedata.bronze.map_vehicle_makes map_vehicle_makes 
    ON stg_rides.vehicle_make_id = map_vehicle_makes.vehicle_make_id

LEFT JOIN realtimedata.bronze.map_vehicle_types map_vehicle_types 
    ON stg_rides.vehicle_type_id = map_vehicle_types.vehicle_type_id

LEFT JOIN realtimedata.bronze.map_ride_statuses map_ride_statuses 
    ON stg_rides.ride_status_id = map_ride_statuses.ride_status_id

LEFT JOIN realtimedata.bronze.map_payment_methods map_payment_methods 
    ON stg_rides.payment_method_id = map_payment_methods.payment_method_id

LEFT JOIN realtimedata.bronze.map_cities map_cities 
    ON stg_rides.pickup_city_id = map_cities.city_id

LEFT JOIN realtimedata.bronze.map_cancellation_reasons map_cancellation_reasons 
