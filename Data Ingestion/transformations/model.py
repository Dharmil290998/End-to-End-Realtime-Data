...
@dp.view
def dim_driver_view():
    df = spark.readStream.table("realtimedata.bronze.silver_obt")
...
def dim_location_view():
    df = spark.readStream.table("realtimedata.bronze.silver_obt")
    df = df.select("pickup_city_id","pickup_city","pickup_city_updated_date","region","state",)
    df = df.dropDuplicates(subset=['pickup_city_id','pickup_city_updated_date'])
    return df
...
dp.create_auto_cdc_flow(
  ...
  keys = ["pickup_city_id"],
  sequence_by = "pickup_city_updated_date",
